...
<script>
  ...
  function getElectricKeyConfig(vin, station){
	
	  $.ajax({
	      url: "materialTrack.sp?getElectricKeyList",
	      type: "POST",
	      dataType: "json",
	      ...
	      success: function(data){
	        ...
	        if(message!=null && message!=""){
	          ...
	          $("#list").text("");
	          for(var i in list){
	            var html = "<tr id='"+list[i].key+"'><td></td><td>"+list[i].description+"</td><td>"+list[i].part_number+"</td><td>"+list[i].barcode+"</td><td>"+list[i].scan_time+"</td><td>"+list[i].scan_user+"</td></tr>";
	            $("#list").append(html);
	          }
	        
	          $("#list tr").each(function(i){
	            var obj = $(this).children(":eq(3)");
	            if(obj.text()!=null && obj.text()!=""){
	              obj.parent().addClass("success");
	              $(this).children(":first").html("<a class='btn btn-danger btn-xs' data-toggle='modal' href='#' " +
  ...
  function getScanList(vin, station){
	//电池包追溯件查询
	if( vin.indexOf("LWF") < 0 ){
		getElectricKeyConfig(vin, station);
		return
	}
  ...
  <div class="panel-body">
    <form class="form-horizontal" role="form">
      <div class="form-group">
        <label class="col-lg-1 col-sm-1 control-label">父物料号</label>
        <div class="col-lg-3 col-sm-3">
          <input class="form-control" id="vin" type="text" onkeypress="scanVin(event)">
        </div>
        <label class="col-lg-1 col-sm-1 control-label">关键件条码</label>
        <div class="col-lg-3 col-sm-3">
