...
function caculateMaterialinit() {
	plantb1 = $('#plantb1').DataTable({
				ordering : true,
				searching : false,
				bStateSave:true,
...
caculateMaterial=function(){
	inputTblRs1 = [];
	 form = new FormData($("#plan")[0]);
	 form.append("queryNum", $("#setNum").val());
	$.ajax({
		url : "materialCaculate.sp?toQuery&t="+Math.random(),
		 type: 'POST',  
         data: form,  
         dataType:'json',
         async: false,  
...
getPlanData=function(){
	inputTblRs = [];
	 ...
	 form.append("queryNum", $("#setNum").val());
	$.ajax({
		url : "materialCaculate.sp?planList&t="+Math.random(),
		 type: 'POST',  
         data: form,  
         dataType:'json',
         async: false,  
