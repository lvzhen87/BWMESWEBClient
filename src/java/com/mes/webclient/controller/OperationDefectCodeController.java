...
@Controller("OperationDefectCodeController")
@RequestMapping("/OperationDefectCodeController.sp")
public class OperationDefectCodeController extends BaseController
{
	@Autowired
	IMService imService;
	...
	{
		return "im/operationDefectCode/list";
	}
	...
	public String toAddOrEdit(OperationDefectCodeVO odcVO, Model model)
	{
		...
		if (key > 0)
		{
			try
			{
				String sql = " select  atr_key, '业务单元', VIN_NUM_S,'order_num' , quality_gate_1_S,quality_gate_2_S ,defect_level_S ,responsible_party_S ,  isnull( part_level1_S ,'') + '/' + isnull( part_level2_S ,'') +'/' +isnull( part_level3_S ,'') +'/'+isnull( part_level4_S ,'') +'/'+isnull( part_level5_S ,'') +'/'+ isnull( PART_name_S ,'')   +'/'+ isnull( level1_S ,'')     +'/'+ isnull( level2_S ,'')  + '/'+ isnull( defect_name_S ,'')   ,  input_user_S   , input_time_T from UD_defect_car WHERE status_S = '1'  and atr_key =  " + key ;
		...
		return "im/operationDefectCode/addOrEdit";
	}
	...
	public String queryList( OperationDefectCodeVO odcVO )
	{
		...
		try
		{
			...
			//工厂分类
			String sql = " select  top "+queryNum+"  atr_key, brand_name_S ,factory_name_S ,'业务单元', VIN_NUM_S,'order_num' , quality_gate_1_S,quality_gate_2_S ,defect_level_S ,responsible_party_S,  isnull( part_level1_S ,'') + '/' + isnull( part_level2_S ,'') +'/' +isnull( part_level3_S ,'') +'/'+isnull( part_level4_S ,'') +'/'+isnull( part_level5_S ,'') +'/'+ isnull( PART_name_S ,'')   +'/'+ isnull( level1_S ,'')     +'/'+ isnull( level2_S ,'')  + '/'+ isnull( defect_name_S ,'')   ,input_user_S ,  CONVERT (varchar,input_time_T ,120 )   from UD_defect_car WHERE status_S = '1'  ";
