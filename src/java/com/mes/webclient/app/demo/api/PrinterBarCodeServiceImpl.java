...
public class PrinterBarCodeServiceImpl implements PrinterBarCodeService {
	...
	public String ExecPassStationFunction(HashMap<String, Object> hsmap) {
		...
		try
		{
		 ...
		 /*   PrintChineseByBmp obj = new PrintChineseByBmp();
			...
			String sendData_S = carTypeDesc + "##" + color + "##" + carProject + "##" + vin;
			ClientSocket client =  new ClientSocket("172.17.12.77", 11111);
			client.sendData(sendData_S);
			client.close();
...
//		String carProject = order.getUDA("car_project") == null ? "" : order.getUDA("car_project").toString();
	 /*   PrintChineseByBmp obj = new PrintChineseByBmp();
		...
		ClientSocket client;
		try {
			client = new ClientSocket("172.18.1.222", 11111);
			client.sendData(sendData_S);
			client.close();
		} catch (Exception e) {
